...
def get_mode_choice():
    while True:
        print("Выберите режим рассылки:")
        print("1 - Автоматическая рассылка по всем каналам")
        print("2 - Рассылка по выбранным каналам (из channels.txt)")
...
async def send_messages(client, last_message, mode, delay_time):
    if mode == 1:
        ...
        if not channels:
            print("Список каналов пуст. Проверьте файл channels.txt.")
            return
        target_channels = []
        for channel in channels:
            try:
                target_channels.append(await client.get_input_entity(channel))
            except Exception as e:
                print(f"Не удалось найти канал: {channel}. Ошибка: {str(e)}")
...
async def start_messaging_cycle(client, delay_time):
    while True:
        ...
        await send_messages(client, last_message, mode, delay_time)
